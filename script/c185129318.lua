...
local c=e:GetHandler()
	...
	if chk==0 then return Duel.IsExistingTarget(s.cfilter,tp,LOCATION_MZONE,0,1,e:GetHandler(),tp)
		and Duel.IsExistingTarget(Card.IsAbleToRemove,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g1=Duel.SelectTarget(tp,s.cfilter,tp,LOCATION_MZONE,0,1,1,e:GetHandler(),tp)
...
function s.activate(e)
	...
	if tc1:IsRelateToEffect(e) and tc1:IsFaceup() and Duel.SendtoHand(tc1,nil,REASON_EFFECT)~=0 and tc2:IsRelateToEffect(e) then
		Duel.Remove(tc2,nil,REASON_EFFECT)
	end
end
--negate effect
