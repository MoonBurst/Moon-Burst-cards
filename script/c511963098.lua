...
function s.initial_effect(c)
	--Fusion summon 1 "Night" fusion monster
	--By banishing appropriate monsters from either GY as material
	local e1=Fusion.CreateSummonEff({handler=c,fusfilter=aux.FilterBoolFunction(Card.IsSetCard,0x196),matfilter=aux.FALSE,extrafil=s.fextra,
...
function s.thcfilter(c,tp)
	return c:IsReason(REASON_BATTLE+REASON_EFFECT) and c:IsSetCard(0x196) and c:IsType(TYPE_FUSION)
