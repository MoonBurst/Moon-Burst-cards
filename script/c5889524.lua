...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,s.thfilter,tp,LOCATION_SZONE,0,1,1,nil,e,tp)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not c:IsOnField() and c:IsPreviousLocation(LOCATION_MZONE) and c:GetPreviousSequence()<5 then
		...
		if Duel.IsExistingMatchingCard(s.zcheck0,tp,LOCATION_MZONE,0,1,nil,zone,tp) then return end
		local e1=Effect.CreateEffect(c)
...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetFlagEffect(id)~=0 and e:GetHandler():GetPreviousLocation(LOCATION_MZONE)
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		local zone=0x1<<c:GetPreviousSequence()
		if Duel.GetMZoneCount(tp)>0 and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)>0 and Duel.GetLocationCount(tp,LOCATION_MZONE,nil,LOCATION_REASON_COUNT)>0 then
