...
function s.initial_effect(c)
	...
	e1:SetCondition(aux.zptcon(s.sp2filter))
	e1:SetTarget(s.sptg)
	...
	e2:SetCondition(aux.zptcon(s.sp2filter2))
	e2:SetTarget(s.sptg)
	e2:SetOperation(s.spop2)
	c:RegisterEffect(e2)
	
	--Lock Set Spell/Trap card
...
function s.spcheck(g,lc,sumtype,tp)
	return g:CheckSameProperty(Card.GetRace,lc,sumtype,tp) and g:CheckDifferentPropertyBinary(Card.GetAttribute,lc,sumtype,tp)
end

-----------------------------------------------------------------------------------------------

function s.sp2filter(c,e,tp)
	return c:IsSetCard(0x1B36) and c:IsLevelBelow(4) and c:IsSummonType(SUMMON_TYPE_SPECIAL)
end
...
----||||||||||||||||||||||||||------------------------------
function s.sp2filter2(c,e,tp)
	return c:IsSetCard(0x1B36) and c:IsSummonType(SUMMON_TYPE_XYZ)
end
