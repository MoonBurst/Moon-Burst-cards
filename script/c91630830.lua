...
function cid.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local b1=Duel.IsExistingMatchingCard(cid.tgfilter,tp,LOCATION_DECK,0,1,nil) and Duel.GetFlagEffect(tp,91630831)==0
	local b2=Duel.IsExistingMatchingCard(cid.thfilter,tp,LOCATION_DECK,0,1,nil) and Duel.GetFlagEffect(tp,91630832)==0
	local b3=Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		and Duel.GetFlagEffect(tp,91630829)==0
...
function cid.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		Duel.RegisterFlagEffect(tp,91630832,RESET_PHASE+PHASE_END,0,1)
	end
end
...
function cid.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP) then
		...
		Duel.RegisterFlagEffect(tp,91630829,RESET_PHASE+PHASE_END,0,1)
	end
