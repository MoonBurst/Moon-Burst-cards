...
function s.counterfilter(c)
	return not c:IsSummonLocation(LOCATION_EXTRA) or c:IsRace(RACE_PSYCHIC)
end
...
function s.splimit(e,c,sump,sumtype,sumpos,targetp,se)
	return not c:IsRace(RACE_PSYCHIC) and c:IsLocation(LOCATION_EXTRA)
end
...
function s.thfilter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsSetCard(0x196) and c:IsAbleToHand()
end
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x196) and c:IsLevel(4) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
