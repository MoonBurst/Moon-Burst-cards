...
function s.filter3(c,rk)
	return c:IsFaceup() and c:IsSetCard(0x196) and c:IsType(TYPE_XYZ) and c:GetRank()>rk
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
    ...
    if sc then
        ...
        local g=Duel.GetMatchingGroup(aux.disfilter3,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,sc)
    local tc=g:GetFirst()
