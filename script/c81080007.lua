...
function cid.condition(e,tp,eg,ep,ev,re,r,rp)
	...
	return ((rp==1-tp and c:IsReason(REASON_EFFECT)) or  c:IsReason(REASON_BATTLE)) and c:GetPreviousControler()==tp
end
--Filters
...
function cid.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetCounter(0x81081)>=10 then
		...
		token:RegisterFlagEffect(0,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(21770264,2))
		local e1=Effect.CreateEffect(c)
	...
	elseif c:GetCounter(0x81081)>=6 then
		...
		e2:SetValue(cid.efilter)
		token:RegisterEffect(e2,true)
		token:RegisterFlagEffect(0,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(21770263,2))
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
		e1:SetCode(EFFECT_CANNOT_BE_LINK_MATERIAL)
		e1:SetValue(aux.TargetBoolFunction(aux.NOT(Card.IsSetCard),0x8108))
