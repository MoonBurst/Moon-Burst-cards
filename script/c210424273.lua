...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0x666,0x666,1500,1500,3,RACE_BEAST,ATTRIBUTE_DARK) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0x666,0x666,1500,1500,3,RACE_BEAST,ATTRIBUTE_DARK) then
...
function s.sumtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCountFromEx(tp)>0
		...
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_EXTRA+LOCATION_GRAVE,0,2,nil,e,tp)			
		end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
end
function s.sumop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCountFromEx(tp)<=0 then return end
	if not Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_EXTRA,0,1,nil,e,tp) then return end
	
	local g2=Duel.SelectMatchingCard(tp,s.filter2,tp,LOCATION_EXTRA+LOCATION_GRAVE,0,2,2,nil,e,tp)
