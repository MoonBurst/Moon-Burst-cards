...
function cid.initial_effect(c)
	...
	if Card.Type then 
	Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x666),2)
...
function cid.negfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_MONSTER) and not c:IsDisabled() and (not c:IsNonEffectMonster() or c:GetOriginalType()&TYPE_EFFECT~=0)
end
...
function cid.colink(e,tp,eg,ep,ev,re,r,rp)
	...
	return lg:IsExists(cid.IsSetCard,1,nil,0x666)
end
...
function cid.negop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetChainInfo(0,CHAININFO_TARGET_CARDS):Filter(cid.IsRelateToEffect,nil,e)
